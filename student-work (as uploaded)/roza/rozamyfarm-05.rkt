...
( define (sahne-2-çiz s) 
        ...
        (nesne-çiz (sahne-2-sepet s )
                   (place-text/v (v 100 50)
                                 (string-append "Yapılacak:20 SKOR:" ( number->string (sahne-2-skor-e s )))
                                 15 "black"
                                 (sahne-2-bg-iki s)))))
...
Tavuklara dikkat et!" 10 "black" sabit-ekran))
...
(define bg-4  ( bitmap "imaj/farm4.jpg"))
   ...
   (nesne-çiz (sahne-4-çiftçi s )
              (nesne-çiz (sahne-4-tavuk s )
                         (nesne-çiz (sahne-4-yumurta s )           
                                    (place-text/v (v 50 50)
                                                  (string-append "Yapılacak:20 SKOR:"
...
( define yaradılış (evren 1
                          ...
                          (sahne-4 bg-4       
                                   ( nesne çiftçi ( v 200 60)( v 0 0 )( v 0 0 ))
...
(define (evren-skor-2-check e )
  (cond
    (( >= (sahne-2-skor-e (evren-sahne-2 e)) 2)
     (evren-sahne-değiştir e 3))
    (else e )))

(define (evren-skor-4-check e )
  (cond
    (( >= (sahne-4-skor-y (evren-sahne-4 e)) 5)
     (evren-sahne-değiştir e 3))
    (else e )))
