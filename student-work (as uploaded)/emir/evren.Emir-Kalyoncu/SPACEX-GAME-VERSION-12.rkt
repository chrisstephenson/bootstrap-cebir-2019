...
(define TEHLİKE1 (scale 0.2(bitmap "imaj/meteor1.png")))
...
(define oyuncu-uzay-genişliği 125)
...
(define (evren-güncelle e)
  (cond
    ((<= (evren-canSayısı e) 0) (evren 
                                (evren-arkaplanı e)
                                (evren-dünya e)
                                (nesne (scale 0.04 (bitmap "imaj/içiboşkalp.png")) (nesne-yer (evren-oyuncu e)) (v 0 0) (v 0 0))
                                (nesne (place-text/v (v 100 65) "oyun" 20 "white" (rectangle 200 50 "solid" "red") ) (v 250 65) (v 0 0) (v 0 0))                           
                                (nesne (place-text/v (v 100 65) "oyun" 20 "white" (rectangle 200 50 "solid" "red") ) (v 750 65) (v 0 0) (v 0 0))
                                (nesne (scale 0.05 (bitmap "imaj/içiboşkalp.png")) (v 1000 650) (v 0 0) (v 0 0))
                                BENZİN-TİMER
                                GENEL-ZAMAN
                                (scale 0.05 (bitmap "imaj/içiboşkalp.png"))
                                (scale 0.05 (bitmap "imaj/içiboşkalp.png"))
                                0))
    (else (evren
           (evren-arkaplanı e)
           ...
           BENZİN-TİMER
           GENEL-ZAMAN
           (if (> (evren-canSayısı e) 0) CAN1 (scale 0.05 (bitmap "imaj/içiboşkalp.png")))
           (if (> (evren-canSayısı e) 1) CAN2 (scale 0.05 (bitmap "imaj/içiboşkalp.png")))
           (canSayısı-güncelle e)))))
; nesne-terse-döndür nesne --> nesne
(define (nesne-terse-döndür n)
  (nesne-fizik-güncelle (nesne (scale 0.05 (bitmap "imaj/içiboşkalp.png")) (nesne-yer n) (v (v-y (nesne-hız n)) (- (v-x (nesne-hız n)) (* 2 (v-x (nesne-hız n))))) (v 0 1))))
...
(define (oyuncuDur e t)
  (if (or (çarpıştı-mı? (evren-t1 e) (evren-oyuncu e)) (çarpıştı-mı? (evren-t2 e) (evren-oyuncu e)) (< (evren-canSayısı e) 0))
      (nesne (scale 0.04(bitmap "imaj/içiboşkalp.png")) (nesne-yer (evren-oyuncu e)) (v 0 0) (v 0 0)) (oyuncu-güncelle (evren-oyuncu e) t)))
...
(define(nesne-tekrarla-amaç e n)
  (if (> (v-y (nesne-yer n)) 683) (nesne-fizik-güncelle (nesne AMAÇ (v (random 900 940) (random 580 683)) (v -19 -24) (v 0 1))) (nesne-fizik-güncelle  (evren-amaç e))))
