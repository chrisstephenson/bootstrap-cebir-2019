...
(STRUCT anime (devamlı değişim-sıklığı zaman resimler))

;anime-çiz: anime ---> görüntü
(ÖRNEK (anime-çiz (anime true 3 13 (list (circle 10 "outline" "light seagreen")
                                        (circle 25 "solid" "red")
...
(define (anime-çiz a)
  (list-ref
       (anime-resimler a)
      (quohent (anime-zaman a)
       (anime-değişim-sıklığı a)) (length (anime-resimler a))))
     
;anime-güncelle: anime ---> anime 


(ÖRNEK (anime-güncelle (anime true 3 11 (list (circle 10 "outline" "light seagreen")
                                        (circle 25 "solid" "red")
                                        (circle 50 "outline" "blue")
                                        (circle 25 "solid" "green")))))

(ÖRNEK (anime-güncelle (anime false 3 11 (list (circle 10 "outline" "light seagreen")
                                        (circle 25 "solid" "red")
                                        (circle 50 "outline" "blue")
                                        (circle 25 "solid" "green")))))

(define (anime-güncelle a)
  (anime
    (anime-devamlı a)
    (anime-değişim-sıklığı a)
    (anime-zaman)


;anime-zaman-güncelle: sayı sayı boolean ---> sayı

(ÖRNEK (anime-zaman-güncelle 11 12 true) 0)
(ÖRNEK (anime-zaman-güncelle 11 12 false) 11)

(define (anime-zaman-güncelle z u 
