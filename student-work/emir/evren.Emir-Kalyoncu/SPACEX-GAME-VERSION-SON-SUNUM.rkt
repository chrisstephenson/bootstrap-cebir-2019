...
(define uzay (bitmap "imaj/uazy2.jpeg"))
(define BACKGROUND (place-image/v (scale 1.5 (bitmap "imaj/earth1.png")) (v 495  785) uzay)) 
...
(define (tehlikeYakınlık n)
  (/ (image-width (nesne-imaj n)) 2))
 
(define level2-BACKGROUND (place-image/v  (scale 0.7(bitmap "imaj/mars.png")) (v 495  785) uzay))
(define oyuncu-uzay-genişliği 100)
   ...
   (place-image/v (evren-can2 e) (v 120 65) (place-image/v (evren-can1 e) (v 35 65) (place-text/v (v 915 65) (number->string (evren-genel-zaman e)) 50 "white" (nesne-çiz (evren-oyuncu e)
             (nesne-çiz (evren-t1 e)
                        (nesne-çiz (evren-t2 e)
                                   (nesne-çiz (evren-amaç e) (evren-arkaplanı e)))))))))
...
(define (çarpıştı-mı? n1 n2)
  (< (v-mag (mesafe-ölç n1 n2)) (tehlikeYakınlık n1)))
; zaman
...
(define (evren-güncelle e)
  (cond
    ((<= (evren-canSayısı e) 0) (evren 
                                ...
                                (nesne (place-text/v (v 80 25) "GAME" 40 "white" (rectangle 200 50 "solid" "red") ) (v 250 65) (v 0 0) (v 0 0))                           
                                (nesne (place-text/v (v 80 25) "OVER" 40 "white" (rectangle 200 50 "solid" "red") ) (v 750 65) (v 0 0) (v 0 0))
                                (nesne (place-text/v (v 180 55) "PRESS SPACE" 40 "white" (ellipse 400 100 "solid" "blue") ) (v 500 340) (v 0 0) (v 0 0))
                                BENZİN-TİMER
                                (evren-genel-zaman e)
                                boşkalp
                                boşkalp
                                0))
    ((> (evren-genel-zaman e) 150) (evren
                                    level2-BACKGROUND
                                    (evren-dünya e)
                                    (evren-oyuncu e)
                                    (if (çarpıştı-mı? (evren-t1 e) (evren-oyuncu e)) (nesne-terse-döndür (evren-t1 e)) (level2-nesne-tekrarla-t1 e (evren-t1 e)))                           
                                    (if (çarpıştı-mı? (evren-t2 e) (evren-oyuncu e)) (nesne-terse-döndür (evren-t2 e)) (nesne-tekrarla-t2 e (evren-t2 e)))
                                    (if (çarpıştı-mı? (evren-amaç e) (evren-oyuncu e)) (nesne-terse-döndür (evren-amaç e)) (level2-nesne-tekrarla-amaç e (evren-amaç e)))
                                    BENZİN-TİMER
    ...
    (else (evren
           ...
           (if (çarpıştı-mı? (evren-t2 e) (evren-oyuncu e)) (nesne-terse-döndür (evren-t2 e)) (nesne-tekrarla-t2 e (evren-t2 e)))
           (if (çarpıştı-mı? (evren-amaç e) (evren-oyuncu e)) (nesne-terse-döndür (evren-amaç e)) (nesne-tekrarla-amaç e (evren-amaç e)))
           BENZİN-TİMER
           (zaman-güncelle (evren-genel-zaman e))
           (if (> (evren-canSayısı e) 0) CAN1 boşkalp)
           (if (> (evren-canSayısı e) 1) CAN2 boşkalp)
           (canSayısı-güncelle e)))))
;(if (< (evren-genel-zaman e) 0) (evren-arkaplanı e) level2-BACKGROUND)
...
(define(level2-nesne-tekrarla-t1 e n)
  (if (> (v-y (nesne-yer n)) 683)  (nesne (scale (random 1 3) TEHLİKE1) (v (random 990 1100) (random 650 683)) (v (random -24 -19) (random -24 -22)) (v 0 1)) (nesne-fizik-güncelle  (evren-t1 e)))) 

  ;(if (> (evren-genel-zaman e) 100))
...
(define(level2-nesne-tekrarla-amaç e n)
  (if (> (v-y (nesne-yer n)) 683)  (nesne (scale (random 1 2) AMAÇ) (v (random 990 1100) (random 200 683)) (v (random -24 -19) (random -24 -22)) (v 0 1)) (nesne-fizik-güncelle  (evren-amaç e))))
