...
(define TEHLIKE (nesne
                 ...
                 (v 1000 445)(v -20 0)(v 0 0)))
(define TEHLIKE2 (nesne
                 (scale 0.15(bitmap "imaj/chara.png"))
                 (v 1700 130)(v -20 0)(v 0 0)))
(define ODUL (nesne         
              (scale 0.3(bitmap "imaj/strawberry.png"))
              (v 800 445)(v -14 0)(v 0 0)))
(define KALP1 (nesne
...
(define (evren-güncelle e)
  (evren
   (evren-bg e)
   (evren-oyuncu e)
   (tehlike-guncelle(nesne-fizik-güncelle (evren-tehlike e)))
   (tehlike-guncelle(nesne-fizik-güncelle (evren-tehlike2 e)))
   (odul-guncelle(nesne-fizik-güncelle (evren-odul e)))
   (evren-kalp1 e)
   (evren-kalp2 e)
   (evren-kalp3 e)))
...
;;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;;oyuncu hareket
;;oyuncu-guncelle nesne metin -> sayi
...
(define arkdis (+ (image-width(evren-bg yaradılış))(/ (image-width(nesne-imaj(evren-tehlike yaradılış))) 2)))
;;tehlike-guncelle nesne -> nesne
(ÖRNEK (tehlike-guncelle (evren-tehlike yaradılış) )
       (nesne
       (nesne-imaj (evren-tehlike yaradılış))
       (cond
          ((and(<= -20 (- 0 (/ (image-width(nesne-imaj (evren-tehlike yaradılış))) 2)))
         (=(v-y (nesne-yer (evren-tehlike yaradılış))) 130))
          ((+ (v-x(nesne-yer (evren-tehlike yaradılış))) arkdis)
          (+ (v-y(nesne-yer (evren-tehlike yaradılış))) 200)))
          
          
          (else (nesne-yer (evren-tehlike yaradılış)))) 
       (nesne-hiz (evren-tehlike yaradılış))
       (nesne-ivme (evren-tehlike yaradılış))))
      
(define (tehlike-guncelle n )
       (nesne
        (nesne-imaj  n)
        (cond
          ((and
         (<= (v-x(nesne-yer n)) (- 0 (/ (image-width(nesne-imaj n)) 2)) )
         (= (v-y (nesne-yer n)) 130)) (v arkdis
                                       (+ (v-y(nesne-yer n)) 210)))
          ((and
         (<= (v-x(nesne-yer n)) (- 0 (/ (image-width(nesne-imaj n)) 2)) )
         (= (v-y (nesne-yer n)) 340)) (v arkdis
                                       (- (v-y(nesne-yer n)) 210)))
          ((and
         (<= (v-x(nesne-yer n)) (- 0 (/ (image-width(nesne-imaj n)) 2)) )
         (= (v-y (nesne-yer n)) 445)) (v arkdis
                                       (- (v-y(nesne-yer n)) 210)))
          ((and
         (<= (v-x(nesne-yer n)) (- 0 (/ (image-width(nesne-imaj n)) 2)) )
         (= (v-y (nesne-yer n)) 235)) (v arkdis
                                       (+ (v-y(nesne-yer n)) 210)))
        ...
        (nesne-hiz n)
        (nesne-ivme n)))
;;odul-guncelle nesne -> nesne
(ÖRNEK (odul-guncelle (evren-odul yaradılış) )
       (nesne
       (nesne-imaj (evren-odul yaradılış))
       (cond
          ((and(<= -20 (- 0 (/ (image-width(nesne-imaj (evren-odul yaradılış))) 2)))
         (=(v-y (nesne-yer (evren-odul yaradılış))) 445))
          (v arkdis
          (- (v-y(nesne-yer (evren-odul yaradılış))) 315)))
          
          
          (else (nesne-yer (evren-odul yaradılış))))
       
       (nesne-hiz (evren-odul yaradılış))
       (nesne-ivme (evren-odul yaradılış))))
(define (odul-guncelle n )
       (nesne
        (nesne-imaj  n)
        (cond
          ((and
